...
STAGE="${STAGE:-v87_edge_pair_support_split_smoke1}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/natural_teacher_support32_actionendpoint_0517/v86_action_edge_pair_samesource_neg_smoke1/final_model.pt}"
if [ ! -f "${INIT_CKPT}" ]; then
  INIT_CKPT="results/natural_teacher_support32_actionendpoint_0517/v85_action_edge_pair_smoke1/final_model.pt"
fi
...
CACHE="${CACHE:-${SAVE_DIR}/segments_v16.pt}"
EPOCHS="${EPOCHS:-1}"
...

{
  ...
  echo "fix=split terminal-support edge-pair head from energy/reward advancing edge-pair head"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --train_action_edge_pair_support_head_only \
  --init_action_edge_pair_support_from_action_edge_pair_head \
  --action_edge_pair_weight 0.0 \
  --prior_action_edge_pair_weight 0.0 \
  --action_edge_pair_support_weight 1.0 \
  --prior_action_edge_pair_support_weight 1.0 \
...
run_eval "edgepair_support_as_change_a32_d4" "action_edge_pair_support_completion" "none" 0 32 4
run_eval "edgepair_support_as_change_a32_d8" "action_edge_pair_support_completion" "none" 0 32 8
run_eval "edgepair_blend_as_change_a32_d4" "action_edge_pair_blend_completion" "none" 0 32 4
run_eval "change_inside_edgepair_support_gate_a32_d4_topk96" "change" "action_edge_pair_support_completion" 96 32 4
run_eval "change_inside_edgepair_support_gate_a32_d4_topk128" "change" "action_edge_pair_support_completion" 128 32 4
...
summary = {
    ...
    "support_head": "terminal changed endpoint edge-pair support",
    "segment_ks": [8, 16, 32],
    "files": {},
}
