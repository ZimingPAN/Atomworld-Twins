...
GPU_ID="${GPU_ID:-0}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/largek_pairfix_0515}"
STAGE="${STAGE:-v56_teacher_contig4_pairprior}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/largek_contig_0515/v56_teacher_contig4/final_model.pt}"
...

{
  ...
  echo "pair_weight=0.5 prior_pair_weight=0.75 count_loss_weight=0.3 prior_edit_weight=0.5 proj_weight=0.75"
} | tee -a "${LOG}"
