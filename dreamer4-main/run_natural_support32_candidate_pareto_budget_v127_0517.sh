...
APPLY_BUDGET="${APPLY_BUDGET:-1}"
STAGE="${STAGE:-v127_candidate_pareto_budget_${PARETO_MODE}_smoke20}"
...
APPLY_ARGS=()
if [ "${APPLY_BUDGET}" = "1" ]; then
  APPLY_ARGS+=(--planner_candidate_pareto_apply_budget_to_projection)
fi

{
  ...
  echo "mode=v127 Pareto selector with budget-to-projection rerun"
  echo "pareto_mode=${PARETO_MODE}"
  echo "pareto_weight=${PARETO_WEIGHT}"
  echo "apply_budget=${APPLY_BUDGET}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
  ...
  --progress_every 5 \
  --device "${DEVICE}" \
  --output "${SAVE_DIR}/eval_long_pareto_budget_${PARETO_MODE}_20.json" \
  2>&1 | tee -a "${LOG}"
...
eval_path = save_dir / "eval_long_pareto_budget_${PARETO_MODE}_20.json"
...
summary = {
    "stage": "${STAGE}",
    "mode": "v127_pareto_budget_to_projection",
