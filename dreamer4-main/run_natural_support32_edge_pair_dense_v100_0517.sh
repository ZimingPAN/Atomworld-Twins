...
STAGE="${STAGE:-v100_dense_legal_pair_selector_smoke1}"
...
if [ ! -f "${CACHE}" ]; then
  CACHE="${SAVE_DIR}/segments_v18.pt"
fi
EPOCHS="${EPOCHS:-1}"
...
ACTION_EDGE_PAIR_DENSE_NEGATIVE_COUNT="${ACTION_EDGE_PAIR_DENSE_NEGATIVE_COUNT:-8}"
...

{
  ...
  echo "action_edge_pair_dense_negative_count=${ACTION_EDGE_PAIR_DENSE_NEGATIVE_COUNT}"
  echo "fix=dense global KMC-legal vacancy->atom negatives for edge energy selector only"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --action_edge_pair_weight 0.50 \
  --prior_action_edge_pair_weight 0.50 \
  --action_edge_pair_support_weight 0.20 \
  --prior_action_edge_pair_support_weight 0.20 \
  --action_edge_pair_semantic_weight 0.40 \
  --prior_action_edge_pair_semantic_weight 0.40 \
  ...
  --action_edge_pair_dense_negative_count "${ACTION_EDGE_PAIR_DENSE_NEGATIVE_COUNT}" \
...
run_eval "edgepair_multi_denselegal_a32_d4" "action_edge_pair_multiobjective_completion" "none" 0
run_eval "twostage_denselegal_a32_d4" "two_stage_vacancy_displacement" "none" 0
run_eval "sequence_denselegal_a32_d4" "terminal_edit_sequence_rollout" "none" 0
run_eval "change_inside_twostage_denselegal_gate96" "change" "two_stage_vacancy_displacement" 96
...
summary = {
    ...
    "action_edge_pair_dense_negative_count": int("${ACTION_EDGE_PAIR_DENSE_NEGATIVE_COUNT}"),
    "edge_selector": "dense global legal negatives for energy head; support head keeps terminal-support targets",
    "segment_ks": [8, 16, 32],
    "files": {},
}
