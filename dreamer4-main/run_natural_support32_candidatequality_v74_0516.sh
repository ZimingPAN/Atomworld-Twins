...
STAGE="${STAGE:-v74_candidatequality_smoke1}"
...
CANDIDATE_QUALITY_WEIGHT="${CANDIDATE_QUALITY_WEIGHT:-1.0}"
PRIOR_CANDIDATE_QUALITY_WEIGHT="${PRIOR_CANDIDATE_QUALITY_WEIGHT:-1.0}"
PLANNER_CANDIDATE_QUALITY_SCORE_WEIGHT="${PLANNER_CANDIDATE_QUALITY_SCORE_WEIGHT:-0.5}"
...

{
  ...
  echo "candidate_quality_weight=${CANDIDATE_QUALITY_WEIGHT}"
  echo "prior_candidate_quality_weight=${PRIOR_CANDIDATE_QUALITY_WEIGHT}"
  echo "planner_candidate_quality_score_weight=${PLANNER_CANDIDATE_QUALITY_SCORE_WEIGHT}"
  echo "fix=learn candidate-level teacher-overlap quality and test quality-aware planner score"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --train_candidate_quality_head_only \
  --candidate_quality_weight "${CANDIDATE_QUALITY_WEIGHT}" \
  --prior_candidate_quality_weight "${PRIOR_CANDIDATE_QUALITY_WEIGHT}" \
  --reward_weight 0.0 \
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
  ...
  --min_projected_changed_sites 2 \
  --device "${DEVICE}" \
  --output "${SAVE_DIR}/eval_long_energy_topk128_qualityscore20.json" \
  2>&1 | tee -a "${LOG}"
...
summary = {
    ...
    "dataset_cache": "${CACHE}",
    "collector": "v69 proposal topk128 + v63 protected reward/tau/risk + teacher-overlap rerank",
    "teacher_overlap_rerank_weight": float("${TEACHER_OVERLAP_RERANK_WEIGHT}"),
    "candidate_quality_weight": float("${CANDIDATE_QUALITY_WEIGHT}"),
    "prior_candidate_quality_weight": float("${PRIOR_CANDIDATE_QUALITY_WEIGHT}"),
    "planner_candidate_quality_score_weight": float("${PLANNER_CANDIDATE_QUALITY_SCORE_WEIGHT}"),
    "segment_ks": [8, 16, 32],
...
for name in [
    ...
    "eval_long_energy_topk128_qualityscore20.json",
]:
    path = save_dir / name
    if not path.exists():
        continue
