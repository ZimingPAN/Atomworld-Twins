...
GPU_ID="${GPU_ID:-2}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/natural_teacher_support32_noopfp_0516}"
STAGE="${STAGE:-v62_projected_noopfp_balanced}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/natural_teacher_support32_editrecover_0516/v61_rewardcal_editrecover_balanced/final_model.pt}"
...

{
  ...
  echo "cache=${CACHE}"
  echo "fix=target-aware projected no-op false-positive penalty plus no-op edit/type copy hard negatives"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
  ...
  --device "${DEVICE}" \
  --output "${SAVE_DIR}/eval_long_min0_80.json" \
  2>&1 | tee -a "${LOG}"
...
summary = {
    ...
    "support": "noopkeep_cache_target_aware_projected_noop_fp",
    "max_candidate_sites": 2048,
    "loss_patch": {
        "noop_change_weight": 0.25,
        "noop_type_copy_weight": 0.10,
