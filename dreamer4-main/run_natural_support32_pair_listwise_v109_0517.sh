...
STAGE="${STAGE:-v109_structured_pair_hardneg_smoke1}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/natural_teacher_support32_sequence_rollout_0517/v101b_terminal_vacancy_pair_selector_smoke1/final_model.pt}"
if [ ! -f "${INIT_CKPT}" ]; then
  INIT_CKPT="results/natural_teacher_support32_sequence_rollout_0517/v100_dense_legal_pair_selector_smoke1/final_model.pt"
fi
...
VACANCY_PAIR_STRUCTURED_NEGATIVE_COUNT="${VACANCY_PAIR_STRUCTURED_NEGATIVE_COUNT:-2}"
...

{
  ...
  echo "cache=${CACHE}"
  echo "mode=terminal vacancy-pair structured hard-negative smoke"
...
run_long_eval "vacancy_rank_structured" "vacancy_pair_completion" "0.5"
run_long_eval "energy_rank_structured" "vacancy_pair_energy_blend_completion" "0.0"
...
tags = ["vacancy_rank_structured", "energy_rank_structured"]
...
summary = {
    ...
    "dataset_cache": "${CACHE}",
    "mode": "terminal vacancy-pair structured hard-negative smoke",
