...
GPU_ID="${GPU_ID:-0}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/largek_projectedtaufix_0515}"
STAGE="${STAGE:-v56_projected_tau_consistent_heads}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/largek_projectionfix_train_0515/v56_projection_fallback_duration/final_model.pt}"
...

{
  ...
  echo "fix=projected reward/duration heads now train and eval on the same projected duration inputs"
} | tee -a "${LOG}"
