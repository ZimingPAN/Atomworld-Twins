...
STAGE="${STAGE:-v83_action_endpoint_smoke1}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/natural_teacher_support32_actionproposal_0516/v81_independent_action_support_smoke1b/final_model.pt}"
PLANNER_CKPT="${PLANNER_CKPT:-results/natural_teacher_support32_actionproposal_0516/v81_independent_action_support_smoke1b/final_model.pt}"
...

{
  ...
  echo "cache=${CACHE}"
  echo "fix=direction-aware source/destination endpoint heads initialized from v81 action_support_head"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --train_action_endpoint_heads_only \
  --init_action_endpoint_from_action_support_head \
  --action_source_support_weight 1.0 \
  --prior_action_source_support_weight 1.0 \
  --action_destination_support_weight 1.0 \
  --prior_action_destination_support_weight 1.0 \
  --action_support_hard_negative_weight 0.25 \
  --action_support_rank_margin_weight 0.10 \
...
for B in 96 128; do
  CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
    ...
    --planner_projection_change_source action_endpoint \
    --planner_projection_topk_source action_endpoint \
    --planner_projection_topk_budget "${B}" \
    --proposal_diagnostic \
    --min_projected_changed_sites 2 \
    --device "${DEVICE}" \
    --output "${SAVE_DIR}/eval_long_endpoint_as_change_topk${B}_20.json" \
    2>&1 | tee -a "${LOG}"
  ...
  CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
    ...
    --min_projected_changed_sites 2 \
    --device "${DEVICE}" \
    --output "${SAVE_DIR}/eval_long_change_inside_endpoint_gate_topk${B}_20.json" \
    2>&1 | tee -a "${LOG}"
done
