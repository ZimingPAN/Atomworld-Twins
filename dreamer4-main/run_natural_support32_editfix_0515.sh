...
GPU_ID="${GPU_ID:-1}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/natural_teacher_support32_editfix_0515}"
STAGE="${STAGE:-v57_adaptive_support32_editfix}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
LOG="${SAVE_DIR}/pipeline.log"
BASE_RUN="${BASE_RUN:-results/natural_teacher_support32_0515/v57_adaptive_support32_noaug}"
INIT_CKPT="${INIT_CKPT:-${BASE_RUN}/final_model.pt}"
DATASET_CACHE="${DATASET_CACHE:-${BASE_RUN}/segments.pt}"
...

{
  ...
  echo "continuation=edit_projection_count_focus freeze_duration_heads support32"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --pair_weight 0.4 \
  --prior_pair_weight 0.5 \
  --prior_edit_weight 1.2 \
  --proj_weight 1.4 \
...
summary = {
    ...
    "continuation": "edit_projection_count_focus_freeze_duration",
    "files": {},
}
for name in [
    "metrics.json",
    ...
    if "reward_sum" in data:
        summary["files"][name]["reward_sum"] = data.get("reward_sum")
    if "tau_expected" in data:
        summary["files"][name]["tau_expected"] = data.get("tau_expected")
    if "val" in data:
        val = data["val"]
        summary["files"][name]["val"] = {
            key: val.get(key)
            for key in [
                "reward_corr",
                "tau_log_mae",
