...
GPU_ID="${GPU_ID:-1}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/natural_teacher_support32_adapt2048_from_fixed1024_0515}"
STAGE="${STAGE:-v58_adaptive2048_from_fixed1024_freeze}"
...

{
  ...
  echo "continuation=adaptive2048_cache_from_fixed1024 conservative_freeze_duration"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --count_loss_weight 0.18 \
  --pair_weight 0.10 \
  --prior_pair_weight 0.12 \
  --prior_edit_weight 0.28 \
  --proj_weight 0.35 \
...
summary = {
    ...
    "max_candidate_sites": 2048,
    "continuation": "adaptive2048_cache_from_fixed1024_conservative_freeze_duration",
    "files": {},
}
summary_keys = [
    "num_samples",
