...
STAGE="${STAGE:-v80_actionrank_smoke1}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/natural_teacher_support32_touchedproposal_0516/v78_touchedsupport_smoke1/final_model.pt}"
PLANNER_CKPT="${PLANNER_CKPT:-results/natural_teacher_support32_touchedproposal_0516/v78_touchedsupport_smoke1/final_model.pt}"
...
PROPOSAL_TARGET_SOURCE="${PROPOSAL_TARGET_SOURCE:-changed_or_touched}"
PROPOSAL_HARD_NEGATIVE_WEIGHT="${PROPOSAL_HARD_NEGATIVE_WEIGHT:-0.5}"
PROPOSAL_RANK_MARGIN_WEIGHT="${PROPOSAL_RANK_MARGIN_WEIGHT:-0.25}"
...

{
  ...
  echo "cache=${CACHE}"
  echo "proposal_target_source=${PROPOSAL_TARGET_SOURCE}"
  echo "teacher_overlap_rerank_weight=${TEACHER_OVERLAP_RERANK_WEIGHT}"
  echo "proposal_hard_negative_weight=${PROPOSAL_HARD_NEGATIVE_WEIGHT}"
  echo "proposal_rank_margin_weight=${PROPOSAL_RANK_MARGIN_WEIGHT}"
  ...
  echo "fix=combine touched action support target with candidate-overlap masks and ranking losses"
} | tee -a "${LOG}"
...
for B in 96 128; do
  CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
    ...
    --device "${DEVICE}" \
    --output "${SAVE_DIR}/eval_long_proposal_as_change_topk${B}_20.json" \
    2>&1 | tee -a "${LOG}"
  ...
  CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
    ...
    --min_projected_changed_sites 2 \
    --device "${DEVICE}" \
    --output "${SAVE_DIR}/eval_long_change_inside_proposal_gate_topk${B}_20.json" \
    2>&1 | tee -a "${LOG}"
done
...
summary = {
    ...
    "dataset_cache": "${CACHE}",
    "proposal_target_source": "${PROPOSAL_TARGET_SOURCE}",
    "collector": "v78 touched proposal topk96 with v63 protected reward/tau/risk plus teacher-overlap rerank and candidate masks",
    "teacher_overlap_rerank_weight": float("${TEACHER_OVERLAP_RERANK_WEIGHT}"),
    "proposal_hard_negative_weight": float("${PROPOSAL_HARD_NEGATIVE_WEIGHT}"),
    "proposal_rank_margin_weight": float("${PROPOSAL_RANK_MARGIN_WEIGHT}"),
