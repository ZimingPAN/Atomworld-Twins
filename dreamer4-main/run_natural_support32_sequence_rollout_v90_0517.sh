...
STAGE="${STAGE:-v90_sequence_rollout_residual_smoke1}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/natural_teacher_support32_terminaldecouple_0517/v89_terminal_sparse_edit_decoupled_smoke1/final_model.pt}"
if [ ! -f "${INIT_CKPT}" ]; then
  INIT_CKPT="results/natural_teacher_support32_actionendpoint_0517/v88_edge_pair_multiobjective_smoke1/final_model.pt"
fi
...
CACHE="${CACHE:-${SAVE_DIR}/segments_v17.pt}"
EPOCHS="${EPOCHS:-1}"
...

{
  ...
  echo "fix=v90 teacher-consistent ordered action rollout as terminal support context"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --terminal_edit_action_context_source teacher_rollout \
...
run_eval "sequence_rollout_as_change_a32_d4" "terminal_edit_sequence_rollout" "none" 0 32 4
run_eval "sequence_inside_rollout_as_change_a32_d4" "terminal_edit_inside_sequence_rollout" "none" 0 32 4
run_eval "sequence_rollout_as_change_a32_d8" "terminal_edit_sequence_rollout" "none" 0 32 8
run_eval "change_inside_sequence_gate_topk96" "change" "terminal_edit_sequence_rollout" 96 32 4
run_eval "edgepair_multi_inside_sequence_gate_topk96" "action_edge_pair_multiobjective_completion" "terminal_edit_sequence_rollout" 96 32 4
