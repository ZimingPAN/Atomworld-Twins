...
STAGE="${STAGE:-v95_vacancy_pair_oracle_readonly}"
...
OUT="${SAVE_DIR}/v95_vacancy_pair_oracle_readonly.json"
...

{
  ...
  echo "cache=${CACHE}"
  echo "diagnostic=v95 read-only pair-level vacancy displacement oracle"
} | tee -a "${LOG}"

"${PYTHON_BIN}" diagnose_vacancy_pair_oracle_v95.py \
  --cache "${CACHE}" \
  --output "${OUT}" \
  ...
  --stage-summary results/natural_teacher_support32_sequence_rollout_0517/v94_terminal_typed_diff_smoke1/stage_summary.json \
