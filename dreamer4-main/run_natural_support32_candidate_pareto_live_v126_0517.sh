...
STAGE="${STAGE:-v126_candidate_pareto_diagnostic_smoke20}"
...
SELECTOR_SPEC="${SELECTOR_SPEC:-results/natural_teacher_support32_sequence_rollout_0517/v123_candidate_pareto_replay_readonly/candidate_pareto_selector_spec_v123.json}"
...

{
  ...
  echo "mode=v126 diagnostic-only live Pareto selector feature smoke"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
  ...
  --progress_every 5 \
  --device "${DEVICE}" \
  --output "${SAVE_DIR}/eval_long_pareto_diagnostic_20.json" \
  2>&1 | tee -a "${LOG}"
...
eval_path = save_dir / "eval_long_pareto_diagnostic_20.json"
...
for segment in segments:
    ...
    candidates = [
        item for item in segment.get("planner_candidates", [])
...
summary = {
    "stage": "${STAGE}",
    "mode": "diagnostic_only_live_pareto_selector",
    "checkpoint": "${CHECKPOINT}",
