...
STAGE="${STAGE:-v73_candidaterank_smoke1}"
...
PROPOSAL_CANDIDATE_POSITIVE_WEIGHT="${PROPOSAL_CANDIDATE_POSITIVE_WEIGHT:-0.8}"
PROPOSAL_CANDIDATE_NEGATIVE_WEIGHT="${PROPOSAL_CANDIDATE_NEGATIVE_WEIGHT:-0.8}"
...

{
  ...
  echo "fix=store multi-candidate teacher-overlap masks and train candidate-level proposal support/ranking"
} | tee -a "${LOG}"
...
summary = {
    ...
    "dataset_cache": "${CACHE}",
    "collector": "v69 proposal topk128 + v63 protected reward/tau/risk + teacher-overlap rerank + candidate masks",
