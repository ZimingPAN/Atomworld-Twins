...
GPU_ID="${GPU_ID:-0}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/largek_projectionfix_train_0515}"
STAGE="${STAGE:-v56_projection_fallback_duration}"
...

{
  ...
  echo "projection=fallback_pair_support duration_source=model eval + baseline eval"
} | tee -a "${LOG}"
