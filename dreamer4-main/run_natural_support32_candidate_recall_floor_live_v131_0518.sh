...
STAGE="${STAGE:-v131_recall_floor_selector_diagnostic_smoke20}"
...

{
  ...
  echo "mode=v131 diagnostic-only live recall-floor selector hook"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
  ...
  --progress_every 5 \
  --device "${DEVICE}" \
  --output "${SAVE_DIR}/eval_long_recall_floor_diagnostic_20.json" \
  2>&1 | tee -a "${LOG}"
...
eval_path = save_dir / "eval_long_recall_floor_diagnostic_20.json"
...
old_selected_budget_hist = Counter()
selector_suggested_k_hist = Counter()
old_selected_k_hist = Counter()
candidate_count_hist = Counter()
old_selection_mismatch_count = 0
selector_suggested_same_as_old_count = 0
candidate_records = 0
selector_records = 0
selected_records = 0
selector_suggested_pred_pair_recall = []
...
for segment in segments:
    ...
    old_selected_k_hist[str(int(segment.get("segment_k", 0)))] += 1
    if old_pick is not None:
        selected_records += 1
        old_selected_budget_hist[
            str(int((old_pick.get("planner_candidate_pareto_selector") or {}).get("budget", -1)))
        ] += 1
        if int(old_pick.get("segment_k", -1)) != int(segment.get("segment_k", -2)):
            old_selection_mismatch_count += 1
    if selector_pick is not None:
        ...
        selector_budget_hist[str(int(selector.get("budget", -1)))] += 1
        selector_floor_pass_hist[str(bool(selector.get("pair_recall_floor_passed", False)))] += 1
        predictions = selector.get("predictions") or {}
        selector_suggested_pred_pair_recall.append(predictions.get("pair_recall", 0.0))
        if old_pick is not None and int(selector_pick.get("segment_k", -1)) == int(old_pick.get("segment_k", -2)):
            selector_suggested_same_as_old_count += 1
...
summary = {
    "stage": "${STAGE}",
    "mode": "diagnostic_only_live_recall_floor_selector",
    "checkpoint": "${CHECKPOINT}",
    ...
    "diagnostic_integrity": {
        ...
        "old_selection_mismatch_count": int(old_selection_mismatch_count),
        "selector_suggested_same_as_old_count": int(selector_suggested_same_as_old_count),
        "old_selected_k_histogram": dict(sorted(old_selected_k_hist.items())),
        "old_selected_budget_histogram": dict(sorted(old_selected_budget_hist.items())),
        ...
        "selector_suggested_pred_pair_recall_mean": mean(selector_suggested_pred_pair_recall),
        "budget_applied_to_projection": bool(selector_summary.get("budget_applied_to_projection", True)),
