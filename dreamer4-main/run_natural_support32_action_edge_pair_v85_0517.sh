...
STAGE="${STAGE:-v85_action_edge_pair_smoke1}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/natural_teacher_support32_actionendpoint_0517/v83_action_endpoint_smoke1b/final_model.pt}"
...
LR="${LR:-8e-5}"
ACTION_EDGE_PAIR_NEGATIVE_MODE="${ACTION_EDGE_PAIR_NEGATIVE_MODE:-self}"
ACTION_EDGE_PAIR_NEGATIVE_WEIGHT="${ACTION_EDGE_PAIR_NEGATIVE_WEIGHT:-1.0}"
ACTION_EDGE_PAIR_RANK_MARGIN_WEIGHT="${ACTION_EDGE_PAIR_RANK_MARGIN_WEIGHT:-0.5}"
...

{
  ...
  echo "fix=edge-pair action generator head for teacher old_pos -> new_pos supervision"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --train_action_edge_pair_head_only \
  --action_edge_pair_weight 1.0 \
  --prior_action_edge_pair_weight 1.0 \
...
run_eval "edgepair_as_change_a32_d4" "action_edge_pair_completion" "none" 0 32 4
run_eval "edgepair_as_change_a32_d8" "action_edge_pair_completion" "none" 0 32 8
run_eval "change_inside_edgepair_gate_a32_d4_topk96" "change" "action_edge_pair_completion" 96 32 4
run_eval "change_inside_edgepair_gate_a32_d4_topk128" "change" "action_edge_pair_completion" 128 32 4
