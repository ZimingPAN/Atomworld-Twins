...
GPU_ID="${GPU_ID:-1}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/natural_teacher_support32_cand2048_0515}"
STAGE="${STAGE:-v58_adaptive_support32_cand2048_noaug}"
...

{
  ...
  echo "init=${INIT_CKPT}"
  echo "segment_ks=8,16,32 adaptive_min_k=8 support=no_teacher_path_aug max_seed_vacancies=32 max_candidate_sites=2048"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --count_loss_weight 0.35 \
  --pair_weight 0.2 \
  --prior_pair_weight 0.25 \
  --prior_edit_weight 0.7 \
  --proj_weight 0.9 \
  --no_aux_anneal \
