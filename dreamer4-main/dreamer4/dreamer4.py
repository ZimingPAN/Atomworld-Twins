...
try:
    ...
    if torch.cuda.is_available() and sys.version_info < (3, 13):
...
class LPIPSLoss(Module):
    def __init__(
        ...
        vgg_weights: VGG16_Weights | None = VGG16_Weights.DEFAULT if VGG16_Weights is not None else None,
        sampled_frames = 1
    ):
        super().__init__()

        if not exists(vgg):
            if torchvision is None or VGG16_Weights is None:
                raise ImportError("LPIPSLoss requires the optional dependency `torchvision`.")
...
class VideoTokenizer(Module):
    def __init__(
        ...
        if encoder_add_decor_aux_loss and DecorrelationLoss is None:
            raise ImportError(
                "encoder_add_decor_aux_loss requires the optional dependency path `vit_pytorch.vit_with_decorr`."
            )
