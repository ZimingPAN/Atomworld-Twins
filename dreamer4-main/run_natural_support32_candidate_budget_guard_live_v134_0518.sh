...
STAGE="${STAGE:-v134_score_scale_minbudget48_diagnostic_smoke20}"
...

{
  ...
  echo "mode=v134 diagnostic-only score-scale normalized recall-floor selector"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" eval_macro_long_trajectory.py \
  ...
  --progress_every 5 \
  --device "${DEVICE}" \
  --output "${SAVE_DIR}/eval_long_score_scale_minbudget48_diagnostic_20.json" \
  2>&1 | tee -a "${LOG}"
...
eval_path = save_dir / "eval_long_score_scale_minbudget48_diagnostic_20.json"
...
summary = {
    "stage": "${STAGE}",
    "mode": "diagnostic_only_score_scale_normalized_min_budget_guard",
    "checkpoint": "${CHECKPOINT}",
