...
STAGE="${STAGE:-v72_overlaphard_train48}"
...
LR="${LR:-5e-5}"
TRAIN_PER_K="${TRAIN_PER_K:-48}"
VAL_PER_K="${VAL_PER_K:-12}"
TEACHER_OVERLAP_RERANK_WEIGHT="${TEACHER_OVERLAP_RERANK_WEIGHT:-0.5}"
PROPOSAL_HARD_NEGATIVE_WEIGHT="${PROPOSAL_HARD_NEGATIVE_WEIGHT:-1.0}"
...

{
  ...
  echo "fix=selected projected false positives become proposal hard negatives with margin ranking"
} | tee -a "${LOG}"
...
summary = {
    ...
    "dataset_cache": "${CACHE}",
    "collector": "v69 proposal topk128 with v63 protected reward/tau/risk plus teacher-overlap rerank",
