...
GPU_ID="${GPU_ID:-1}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/natural_teacher_support32_fixed_0515}"
STAGE="${STAGE:-v57_fixed_support32_noaug}"
...

{
  ...
  echo "init=${INIT_CKPT}"
  echo "segment_ks=8,16,32 boundary=fixed_k support=no_teacher_path_aug max_seed_vacancies=32"
} | tee -a "${LOG}"
