...
STAGE="${STAGE:-v94_terminal_typed_diff_smoke1}"
...
if [ ! -f "${INIT_CKPT}" ]; then
  INIT_CKPT="results/natural_teacher_support32_terminaldecouple_0517/v89_terminal_sparse_edit_decoupled_smoke1/final_model.pt"
fi
...

{
  ...
  echo "fix=v94 independent terminal typed-diff decoder for vacancy displacement"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --terminal_typed_diff_copy_weight 0.05 \
