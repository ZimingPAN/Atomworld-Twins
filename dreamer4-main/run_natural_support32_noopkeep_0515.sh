...
GPU_ID="${GPU_ID:-2}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/natural_teacher_support32_noopkeep_0515}"
STAGE="${STAGE:-v59_adaptive2048_noopkeep_hardneg}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/natural_teacher_support32_noopsupport_0515/v58_adaptive2048_noop_support/final_model.pt}"
...

{
  ...
  echo "cache=${CACHE}"
  echo "fix=adaptive support32 no-op hard negatives by keeping accepted no-op states within short rollouts"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --reward_zero_weight 4.0 \
  --reward_sign_weight 0.35 \
  --prior_edit_weight 0.55 \
...
summary = {
    ...
    "support": "planner_selected_noop_keep_hardneg_no_teacher_path_aug",
    "max_candidate_sites": 2048,
    "files": {},
}
summary_keys = [
...
for name in [
    ...
    if "cumulative" in data:
        item["cumulative"] = data["cumulative"]
