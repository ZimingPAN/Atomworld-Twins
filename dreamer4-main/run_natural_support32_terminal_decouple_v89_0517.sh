...
GPU_ID="${GPU_ID:-0}"
DEVICE="${DEVICE:-cuda}"
RESULT_ROOT="${RESULT_ROOT:-results/natural_teacher_support32_terminaldecouple_0517}"
STAGE="${STAGE:-v89_terminal_sparse_edit_decoupled_smoke1}"
SAVE_DIR="${RESULT_ROOT}/${STAGE}"
INIT_CKPT="${INIT_CKPT:-results/natural_teacher_support32_actionendpoint_0517/v88_edge_pair_multiobjective_smoke1/final_model.pt}"
if [ ! -f "${INIT_CKPT}" ]; then
  INIT_CKPT="results/natural_teacher_support32_actionendpoint_0517/v87_edge_pair_support_listwise_smoke1/final_model.pt"
fi
...

{
  ...
  echo "fix=v89 decouple energy action sequence from terminal sparse-edit support decoder"
} | tee -a "${LOG}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" train_dreamer_macro_edit.py \
  ...
  --train_terminal_edit_support_head_only \
  --terminal_edit_support_weight 1.0 \
  --prior_terminal_edit_support_weight 1.0 \
  --terminal_edit_support_target_source changed \
  --terminal_edit_support_hard_negative_weight 0.5 \
  --terminal_edit_support_rank_margin_weight 0.25 \
  --terminal_edit_support_candidate_positive_weight 0.5 \
  --terminal_edit_support_candidate_negative_weight 0.5 \
  --terminal_edit_support_candidate_rank_margin_weight 0.25 \
...
run_eval "terminal_decoupled_as_change_a32_d4" "terminal_edit_decoupled" "none" 0 32 4
run_eval "terminal_inside_edge_as_change_a32_d4" "terminal_edit_inside_action_edge" "none" 0 32 4
run_eval "terminal_decoupled_as_change_a32_d8" "terminal_edit_decoupled" "none" 0 32 8
run_eval "change_inside_terminal_decoupled_gate_topk96" "change" "terminal_edit_decoupled" 96 32 4
run_eval "edgepair_multi_inside_terminal_gate_topk96" "action_edge_pair_multiobjective_completion" "terminal_edit_decoupled" 96 32 4
...
summary = {
    ...
    "dataset_cache": "${CACHE}",
    "decoupled_heads": ["edge-pair action sequence", "terminal sparse-edit support"],
