...
| 阶段 3：不同温度、不同成分、不同 lattice size 条件材料演化计算。 | 通过 | 温度 250 K/300 K/350 K/400 K/500 K/600 K/700 K/800 K/900 K/1000 K，Cu density 0.0025/0.005/0.01/0.0134/0.02/0.03/0.05，V density 0.0005/0.001/0.002/0.003/0.005；lattice size 8x8x8, 10x10x10, 12x12x12, 14x14x14, 16x16x16, 18x18x18, 20x20x20, 22x22x22, 24x24x24, 26x26x26；演化图含 temperature/Cu/vacancy 三个面板。 |
| 阶段 3：不少于百节点 DeepH 和 DeepKS 并行展示。 | 通过 | `parallel_training_display.csv` 覆盖 1 到 1024 节点并行扩展性估算记录。 |
| 阶段 4：优化前后效率、不同规模运行时间和并行效率。 | 通过 | 10 种 lattice size，baseline=2048 次全量刷新，最大 measured speedup=268.658x。 |
...
| 主要输出 4：跨尺度数据集。 | 通过 | `outputs/datasets/multiscale_dataset.csv` 与 `outputs/tables/multiscale_dataset.csv`。 |
...
| 验收展示：材料演化结果。 | 通过 | 演化图展示 energy delta、Cu cluster 和 vacancy density 物理时间面板；逐步非零能量变化 9896/36000。 |
...
- 性能合理性：10 种 lattice size 均完成 baseline/optimized 对比，baseline=2048 次全量刷新，最大 measured speedup=268.658x；并行表覆盖 DeepH/DeepKS 的 1--1024 节点扩展性估算配置。
