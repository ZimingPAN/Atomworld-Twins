...
| 验证并行计算展示能力。 | 通过 | `parallel_training_display.csv` 覆盖 DeepH/DeepKS 并行扩展性估算记录，最大节点数 128。 |
...
| 阶段 4：优化前后效率、不同规模运行时间和并行效率。 | 通过 | 8/10/12 三种规模，最大 measured speedup=1.322x。 |
...
- 性能合理性：三种规模均完成 baseline/optimized 对比，最大 measured speedup=1.322x；并行表覆盖 DeepH/DeepKS 的 1--128 节点扩展性估算配置。
...
- `figures/material_evolution_curves.png`：尺寸 2196x936，非空内容区域 (161, 74, 1977, 929)，mean_diff=8.60，判定：非空且可视。
- `figures/cu_cluster_structure.png`：尺寸 1170x990，非空内容区域 (167, 8, 1109, 976)，mean_diff=10.70，判定：非空且可视。
- `figures/runtime_comparison.png`：尺寸 1350x756，非空内容区域 (7, 8, 1343, 743)，mean_diff=51.68，判定：非空且可视。
