...
| 文档句子 / 条目 | 结论 | 证据或边界 |
| --- | --- | --- |
| 测试目标：以 Fe-Cu-vacancy 作为测试样例。 | 通过 | 350 个 Fe-Cu-vacancy 温度/成分/缺陷组合进入 KMC 演化。 |
| 验证材料能量计算能力。 | 通过 | `energy_results.csv` 有 350 行；284 个组合出现非零能量变化。 |
| 验证跨尺度数据生成能力。 | 通过 | `multiscale_dataset.csv` 有 35000 条逐步 KMC 记录。 |
...
| 阶段 3：不同温度、不同成分条件材料演化计算。 | 通过 | 温度 250 K/300 K/350 K/400 K/500 K/600 K/700 K/800 K/900 K/1000 K，Cu density 0.0025/0.005/0.01/0.0134/0.02/0.03/0.05，V density 0.0005/0.001/0.002/0.003/0.005。 |
| 阶段 3：不少于百节点 DeepH 和 DeepKS 并行展示。 | 通过 | `parallel_training_display.csv` 覆盖 1 到 1024 节点并行扩展性估算记录。 |
| 阶段 4：优化前后效率、不同规模运行时间和并行效率。 | 通过 | 8/10/12 三种规模，baseline=64 次全量刷新，最大 measured speedup=11.857x。 |
...
| 验收展示：材料演化结果。 | 通过 | 演化图展示 energy delta 和 Cu cluster；逐步非零能量变化 9598/35000。 |
| 验收展示：设计优化结果。 | 通过 | 建议文本明确按成分/配方/显微组织调控给出。 |
| 验收展示：验收报告。 | 通过 | Markdown 与 DOCX 双格式。 |

## 输出合理性检查

- 数据规模：350 个跨尺度组合，每个 100 个 KMC step，共 35000 条逐步记录。
- 能量合理性：初末 pair energy 范围为 -14802.836887 到 -14434.278865 eV；短步长下部分组合能量近似不变，属于小规模展示边界。
- 组织合理性：final Cu max cluster 范围为 1 到 167，随 Cu density 增大有明显组织差异。
- 性能合理性：三种规模均完成 baseline/optimized 对比，baseline=64 次全量刷新，最大 measured speedup=11.857x；并行表覆盖 DeepH/DeepKS 的 1--1024 节点扩展性估算配置。
...
- `figures/material_evolution_curves.png`：尺寸 2196x936，非空内容区域 (161, 74, 1977, 890)，mean_diff=8.74，判定：非空且可视。
- `figures/cu_cluster_structure.png`：尺寸 1170x990，非空内容区域 (170, 8, 1109, 977)，mean_diff=14.70，判定：非空且可视。
- `figures/runtime_comparison.png`：尺寸 1350x756，非空内容区域 (7, 8, 1343, 743)，mean_diff=46.40，判定：非空且可视。
