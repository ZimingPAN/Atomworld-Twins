...
| 文档句子 / 条目 | 结论 | 证据或边界 |
| --- | --- | --- |
| 测试目标：以 Fe-Cu-vacancy 作为测试样例。 | 通过 | 360 个 Fe-Cu-vacancy 算例进入 KMC 演化，其中温度/成分/缺陷组合 350 个，lattice size 扫描 10 个。 |
| 验证材料能量计算能力。 | 通过 | `energy_results.csv` 有 360 行；293 个组合出现非零能量变化。 |
| 验证跨尺度数据生成能力。 | 通过 | `multiscale_dataset.csv` 有 36000 条逐步 KMC 记录。 |
| 验证并行计算展示能力。 | 通过 | `parallel_training_display.csv` 覆盖 DeepH/DeepKS 并行扩展性估算记录，最大节点数 1024。 |
...
| 阶段 3：不同温度、不同成分、不同 lattice size 条件材料演化计算。 | 通过 | 温度 250 K/300 K/350 K/400 K/500 K/600 K/700 K/800 K/900 K/1000 K，Cu density 0.0025/0.005/0.01/0.0134/0.02/0.03/0.05，V density 0.0005/0.001/0.002/0.003/0.005；lattice size 8x8x8, 10x10x10, 12x12x12, 14x14x14, 16x16x16, 18x18x18, 20x20x20, 22x22x22, 24x24x24, 26x26x26。 |
| 阶段 3：不少于百节点 DeepH 和 DeepKS 并行展示。 | 通过 | `parallel_training_display.csv` 覆盖 1 到 1024 节点并行扩展性估算记录。 |
| 阶段 4：优化前后效率、不同规模运行时间和并行效率。 | 通过 | 10 种 lattice size，baseline=2048 次全量刷新，最大 measured speedup=279.854x。 |
...
| 主要输出 2b：lattice size 扫描结果表。 | 通过 | `outputs/tables/lattice_size_scan.csv`。 |
...
| 主要输出 9：项目验收报告。 | 通过 | `outputs/reports/acceptance_report.md`、`.tex` 和 `.pdf`。 |
...
| 验收展示：材料演化结果。 | 通过 | 演化图展示 energy delta 和 Cu cluster；逐步非零能量变化 9896/36000。 |
| 验收展示：设计优化结果。 | 通过 | 建议文本明确按成分/配方/显微组织调控给出。 |
| 验收展示：验收报告。 | 通过 | Markdown、LaTeX 与 PDF 格式。 |

## 输出合理性检查

- 数据规模：温度/成分/缺陷组合 350 个，lattice size 扫描 10 个，总计 360 个 KMC 算例；每个 100 个 KMC step，共 36000 条逐步记录。
- 能量合理性：初末 pair energy 范围为 -149825.616261 到 -4364.526262 eV；短步长下部分组合能量近似不变，属于小规模展示边界。
- 组织合理性：final Cu max cluster 范围为 1 到 167，随 Cu density 增大有明显组织差异。
- 性能合理性：10 种 lattice size 均完成 baseline/optimized 对比，baseline=2048 次全量刷新，最大 measured speedup=279.854x；并行表覆盖 DeepH/DeepKS 的 1--1024 节点扩展性估算配置。
...
- `figures/material_evolution_curves.png`：尺寸 2196x936，非空内容区域 (161, 74, 1977, 890)，mean_diff=8.69，判定：非空且可视。
- `figures/cu_cluster_structure.png`：尺寸 1170x990，非空内容区域 (169, 8, 1109, 971)，mean_diff=17.80，判定：非空且可视。
- `figures/runtime_comparison.png`：尺寸 2232x864，非空内容区域 (7, 8, 2225, 851)，mean_diff=17.15，判定：非空且可视。
